...
class CrawlerServiceApplicationTest {
    ...
    Supplier<CrawlerConfig> updatedCrawlerConfig =
            () -> CrawlerConfig.builder().action(crawlerAction.get())
                    ...
                    .startUrls(updatedStartUrls.get()).schedule("0 0 2 1 1 ? 2099").robotOptions(updatedRobotOptions.get()).build();
    ...
    void whenGetCrawlerStatusRequest_thenCrawlerStatusIsReturned() throws JsonProcessingException {
        ...
        webTestClient.get().uri("/crawlers/" + crawlerResponse.getId() + "/status").exchange().expectStatus().isOk()
                .expectBody(StatusResponse.class).value(crawlerStatusResponse -> {
                    assertThat(crawlerResponse.getStatus()).isEqualTo(crawlerStatusResponse.status());
                });
    }


    @Test
    void whenGetCrawlerActionRequest_thenCrawlerActionIsReturned() throws JsonProcessingException {
        var crawlerResponse = webTestClient.post().uri("/crawlers").contentType(MediaType.APPLICATION_JSON)
                .bodyValue(objectMapper.writeValueAsString(crawlerRequest.get())).exchange().expectStatus()
                .isCreated().expectBody(CrawlerResponse.class).returnResult().getResponseBody();
        assertNotNull(crawlerResponse);

        webTestClient.get().uri("/crawlers/" + crawlerResponse.getId() + "/action").exchange().expectStatus().isOk()
                .expectBody(CrawlerAction.class).value(crawlerActionResponse -> {
                    assertThat(crawlerResponse.getConfig().getAction()).isEqualTo(crawlerActionResponse);
                });
    }
